...
def delete_course(course_id):
    global cursos
    cursos = [course for course in cursos if course['id'] != course_id]
...
def delete_testimonial(testimonial_id):
    global depoimentos
    depoimentos = [testimonial for testimonial in depoimentos if testimonial['id'] != testimonial_id]
...
def update_team_member(member_id):
    ...
    return jsonify({'error': 'Membro not found'}), 404

@app.route('/api/team/<int:member_id>', methods=['DELETE'])
def delete_team_member(member_id):
    global equipe
    equipe = [member for member in equipe if member['id'] != member_id]
